//! SHA-256 and HMAC-SHA256 (FIPS 180-4, RFC 2104), implemented on the
//! standard library so webhook verification needs no extra dependency.
...
/// HMAC-SHA256 of `message` keyed with `key`.
...
/// Lowercase hex encoding (tests only).
#[cfg(test)]
